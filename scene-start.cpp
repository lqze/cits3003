...
*	CITS3003-1 2016
*	PROJECT
*	CALEB FETZER (21384976)
*	ELIJAH FETZER (21516694)
*	GROUP# 18	
******/
#include "Angel.h"
...
typedef struct {
    ...
    int texId;
    float texScale;
    float moveSpeed, moveDist; // Animated object movement speed and distance
} SceneObject;
...
int animType = 0; // The animation type for animated models
bool pauseAnim = false; // If animation is paused or not check
float timeAtPause = 0; // The time the object was paused
float timeAtResume = 0; // The time the object was resumed
...
void loadMeshIfNotAlreadyLoaded(int meshNumber) {
    ...
    GLuint buffers[2];
    glGenBuffers( 2, buffers );  // Add two vertex buffer objectsunits/CITS3003/lectures/code-22/vPart2.glsl
    ...
    glBufferData( GL_ARRAY_BUFFER, sizeof(float)*4*nVerts, boneWeights, GL_STATIC_DRAW );
...
static void addObject(int id) {
    ...
    // Apply default values object id's above 56 (i.e. that are animated)
    if (id >= 56) {
        sceneObjs[nObjects].moveSpeed = 0.05;
        sceneObjs[nObjects].moveDist = 4.0;
                
      if (pauseAnim == true) {
	  animStartTime = glutGet(GLUT_ELAPSED_TIME) / 100.0;
      } else {
	  animStartTime = timeAtPause;
      }
    } else {
...
//------Part I: Caleb - Delete an object from the scene--------------------------
...
//------Part I: Elijah - Duplicate current object in the scene------------------
...
static void loadSavedScene(int id) {
	char fname[64];
...
void drawMesh(SceneObject sceneObj) {
    ...
    vec4 displacement = 0;
    float POSE_TIME = 0; 
    float elapsedTime = 0; // Time from beginning of glut_init to when the animation is paused
    float yFunc = 0; // Pass to the displacement vector for displacement in the y-axis
    float zFunc = 0; // Pass to the displacement vector for displacement in the z-axis
    float numFrames = 0;
    float timePeriod = sceneObj.moveDist / sceneObj.moveSpeed; // The ratio of 
	...
	POSE_TIME = fmod(elapsedTime, numFrames);
        ...
        zFunc = sceneObj.moveDist * sin(float(elapsedTime) / timePeriod * 0.5 * M_PI);
        
	switch (animType) {
	    // bouncing
	    case 1:  yFunc = abs(zFunc) * 0.5; 
		     break;
	    // straight
	    default: yFunc = 0.0;
		     break;
    ...
    // For Part B: Combine translation, rotation (above), and scaling based on what's in the sceneObj struct
    mat4 model = Translate(sceneObj.loc + displacement) * rotateXYZ * Scale(sceneObj.scale);
...
void display( void ) {
    ...
    lightObj2.loc.w = 0.0; // For Part I: set w=0 for distant light source
...
static void mainmenu(int id) {
    ...
    if (id == 59 && currObject >= 0) {
	pauseAnim = false;
	timeAtResume = glutGet(GLUT_ELAPSED_TIME);
	animStartTime += (timeAtResume - timeAtPause);
        glutPostRedisplay();
    }
    if (id == 99) 
	exit(EXIT_SUCCESS);
...
static void makeMenu() {
    ...
    // Part K: Create a menu item for loading and saving scenes
    int no_entries = 20;
    char scenes[no_entries][128];
    for (int i = 0; i < no_entries; i++)
	sprintf(scenes[i], "Scene%d", i+1);
	int loadMenuId = createArrayMenu(no_entries, scenes, loadSavedScene);
	int saveMenuId = createArrayMenu(no_entries, scenes, saveScene);
    ...
    glutAddMenuEntry("Walking Distance/Speed", 57);
    glutAddMenuEntry("Animation Bounce On", 60);
    glutAddMenuEntry("Animation Bounce Off", 61);
    glutAddMenuEntry("Pause animation", 58);
    glutAddMenuEntry("Resume animation", 59);
